...
import {addCodingReview } from "../services/postRequest";
import { useNavigate } from "react-router-dom";

const CodingRoundReviewForm = (props) => {
  ...
  const[codingReviewDetail,setCodingReviewDetail] = useState({
 ...
 const OnHandleAddValueChange =(event)=>{
  ...
  const addValueToChecks =(type)=>{
   ...
   const OnHandleChange =(event)=>{
       ...
       setCodingReviewDetail({...codingReviewDetail,[name]:value});
   }
  const onHandleSubmit = (event) => {
    event.preventDefault();
    setSpinner(true);
    codingReviewDetail.typeOfQuestions = checkBoxValues;
    setCodingReviewDetail(codingReviewDetail);
    console.log(codingReviewDetail);
    
    addCodingReview(codingReviewDetail).then((res)=>{

        if(res.data.Success){
            
           window.alert(res.data.Message);
           if(codingReviewDetail.status === "Rejected"){
               navigate('/');
               return;
           }
           setSpinner(false);
          ...
          <Form style={{ marginTop: "3%" }} onSubmit={onHandleSubmit}>
            <Form.Group as={Row} className="mb-3">
              ...
              <Col sm={3}>
                  ...
                  <Col >
                  <Form.Check name="level" style={{marginTop:"3%"}} onChange={OnHandleChange} value="Easy" checked={codingReviewDetail.level==="Easy"} type="radio"  label="Easy" required/>
                  <Form.Check name="level" style={{marginTop:"3%"}} onChange={OnHandleChange} value="Medium" checked={codingReviewDetail.level==="Medium"} type="radio"  label="Medium" required/>
                  <Form.Check name="level" style={{marginTop:"3%"}} onChange={OnHandleChange}  value="Hard" checked={codingReviewDetail.level==="Hard"} type="radio"  label="Hard" required/>
                  </Col>
                  </Row>
              </Col>
              <Col sm={4}>
                  ...
                  <Col >
                  <Form.Control as="input" type="number" min={1}  onChange={OnHandleChange}  style={{marginTop:"3%"}} name="noOfQuestions" value={codingReviewDetail.noOfQuestions}  placeholder="No of Questions" required>
                    </Form.Control>
                  </Col>
                  </Row>
                  <Row>
                  ...
                  <Col >
                  <Form.Control as="input" type="text"  onChange={OnHandleChange}  style={{marginTop:"3%"}} name="timeGiven" value={codingReviewDetail.timeGiven}  placeholder="Time Duration" required>
                  </Form.Control>
                  </Col>
                  </Row>
                  <Row>
                  ...
                  <Col >
                  <Form.Control as="textarea" type="textarea"  onChange={OnHandleChange}  style={{marginTop:"3%"}} name="feedBack" value={codingReviewDetail.feedBack}  placeholder="ex: Overall Time 90 Questions" >
                  </Form.Control>
                  </Col>
                  </Row>
                  <Row>
                  ...
                  <Form.Control
                      ...
                      value={codingReviewDetail.status}
                      style={{marginTop:"3%"}}
                      required
                    >
...
export default CodingRoundReviewForm;
