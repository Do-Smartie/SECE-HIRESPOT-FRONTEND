...
import {addTechHrReview } from "../services/postRequest";
import { useNavigate } from "react-router-dom";

const TechHrReviewForm = (props) => {
  ...
  const[techHrReviewDetail,setTechHrReviewDetail] = useState({
 ...
 const OnHandleAddValueChange =(event)=>{
  ...
  const addValueToChecks =(type)=>{
   ...
   const OnHandleChange =(event)=>{
       ...
       setTechHrReviewDetail({...techHrReviewDetail,[name]:value});
   }
  const onHandleSubmit = (event) => {
    event.preventDefault();
    setSpinner(true);
    techHrReviewDetail.typeOfQuestions = checkBoxValues;
    setTechHrReviewDetail(techHrReviewDetail);
    console.log(techHrReviewDetail);
    
    addTechHrReview(techHrReviewDetail).then((res)=>{

        if(res.data.Success){
            
           window.alert(res.data.Message);
           if(techHrReviewDetail.status === "Rejected"){
               navigate('/');
               return;
           }
           setSpinner(false);
          ...
          <Form style={{ marginTop: "3%" }} onSubmit={onHandleSubmit}>
            <Form.Group as={Row} className="mb-3">
              ...
              <Col sm={3}>
                  ...
                  <Col >
                  <Form.Check name="level" style={{marginTop:"3%"}} onChange={OnHandleChange} value="Easy" checked={techHrReviewDetail.level==="Easy"} type="radio"  label="Easy" required/>
                  <Form.Check name="level" style={{marginTop:"3%"}} onChange={OnHandleChange} value="Medium" checked={techHrReviewDetail.level==="Medium"} type="radio"  label="Medium" required/>
                  <Form.Check name="level" style={{marginTop:"3%"}} onChange={OnHandleChange}  value="Hard" checked={techHrReviewDetail.level==="Hard"} type="radio"  label="Hard" required/>
                  </Col>
                  </Row>
              </Col>
              <Col sm={4}>
                  ...
                  <Col >
                  <Form.Control as="input" type="text"  onChange={OnHandleChange}  style={{marginTop:"3%"}} name="interviewDuration" value={techHrReviewDetail.interviewDuration}  placeholder="Time Duration" required>
                  </Form.Control>
                  </Col>
                  </Row>
                  <Row>
                  ...
                  <Col >
                  <Form.Control as="textarea" type="textarea"  onChange={OnHandleChange}  style={{marginTop:"3%"}} name="feedBack" value={techHrReviewDetail.feedBack}  placeholder="ex: Overall Time 90 Questions" >
                  </Form.Control>
                  </Col>
                  </Row>
                  <Row>
                  ...
                  <Form.Control
                      ...
                      value={techHrReviewDetail.status}
                      style={{marginTop:"3%"}}
                      required
                    >
...
export default TechHrReviewForm;
